...
def perform_forecasting(energy_prices, bodycote_data):
    ...
    bodycote_prophet_data = bodycote_prophet_data[['ds', 'y']]  # Select only 'ds' and 'y'
    ...
    future_revenue = revenue_model.make_future_dataframe(periods=forecast_period, freq='Y')
    ...
    electricity_data = electricity_data[['ds', 'y']]  # Select only 'ds' and 'y'
    electricity_model = Prophet(yearly_seasonality=True)
    electricity_model.fit(electricity_data)
    future_energy = electricity_model.make_future_dataframe(periods=forecast_period, freq='Y')
    ...
    gas_data = gas_data[['ds', 'y']]  # Select only 'ds' and 'y'
    gas_model = Prophet(yearly_seasonality=True)
    gas_model.fit(gas_data)
    future_energy_gas = gas_model.make_future_dataframe(periods=forecast_period, freq='Y')
    gas_forecast = gas_model.predict(future_energy_gas)

    # Combine forecasts
    revenue_forecast = revenue_forecast[['ds', 'yhat']].rename(columns={'yhat': 'revenue_forecast'})
    electricity_forecast = electricity_forecast[['ds', 'yhat']].rename(columns={'yhat': 'electricity_price_forecast'})
    gas_forecast = gas_forecast[['ds', 'yhat']].rename(columns={'yhat': 'gas_price_forecast'})

    combined_forecast = revenue_forecast.merge(electricity_forecast, on='ds').merge(gas_forecast, on='ds')
